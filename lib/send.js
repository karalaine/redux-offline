...
var complete = function complete(action, success, payload, offlineAction) {
  if (success) {
    (0, _offlineActionTracker.resolveAction)(offlineAction.meta.transaction, payload);
  } else {
    (0, _offlineActionTracker.rejectAction)(offlineAction.meta.transaction, payload);
  }
...
var send = function send(action, dispatch, config) {
  ...
  return config.effect(metadata.effect, action).then(function (result) {
    ...
    try {
      ...
      return dispatch(complete({ type: _constants.JS_ERROR }, false, e, action));
    }
  }).catch(function () {
      ...
      return _regenerator2.default.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 15:
              return _context.abrupt('return', dispatch(complete(rollbackAction, false, error, action)));
